        ...
        >
          ...
          <div className="flex items-center gap-1">
            {!isNodeValid(proyecto, 'PROYECTO') && (
              <AlertTriangle className="w-4 h-4 text-red-500 shrink-0" title="Falta información requerida" />
            )}
            <AddSitioDialog />
          </div>
        </div>
            ...
            <div key={sitio.id} className="space-y-1">
              ...
              >
                <div className="flex items-center gap-1.5 overflow-hidden">
                  ...
                  <span className="truncate">{sitio.nombre || 'Nuevo Sitio'}</span>
                </div>
                <div className="flex items-center gap-1">
                  {!isNodeValid(sitio, 'SITIO') && (
                    <AlertTriangle className="w-3.5 h-3.5 text-red-500 shrink-0 mr-1" title="Falta información requerida" />
                  )}
                  <AddPuestoDialog sitioId={sitio.id} />
                </div>
              </div>

              {/* Puestos */}
              <div className="ml-3 border-l border-slate-200 pl-2 space-y-1">
                {sitio.puestos.map(puesto => (
                  <div key={puesto.id} className="space-y-1">
                    ...
                    >
                      <div className="flex items-center gap-1.5 overflow-hidden">
                        ...
                        <span className="truncate text-[13px]">{puesto.nombre || 'Nuevo Puesto'}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        {!isNodeValid(puesto, 'PUESTO') && (
                          <AlertTriangle className="w-3.5 h-3.5 text-red-500 shrink-0" title="Falta información requerida" />
                        )}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1">
                            {!isNodeValid(recurso, 'RECURSO') && (
                              <AlertTriangle className="w-3 h-3 text-red-500 shrink-0" title="Falta información requerida" />
                            )}
                            {recurso.cantidad > 1 && (
